...
- `headers`: array of `{key,value}`; sensitive values (API keys) masked in UI logs but stored encrypted by backend. If docs show placeholder secrets (e.g., `YOUR_SECRET_TOKEN`, `YOUR_API_KEY`), omit the header entirely unless a real value is provided—never persist placeholder strings in the value field.
